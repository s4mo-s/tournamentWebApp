...
export class Ver2Component implements OnInit {
        ...
        if (spider_array && spider_array.length > 0) {
            spider_array.forEach(async (element) => {
                ...
                if (this.tournament.mode == 1) {
                    this.restUser
                        .getUserById(this.match.firstTeam.toString())
                        .subscribe(async (resA) => {
                            this.restUser
                                .getUserById(this.match.secondTeam.toString())
                                .subscribe(async (resB) => {
                                    matches[mat.order - 1] = new myMatch(
                                        mat.id,
                                        mat.order,
                                        resA.username,
                                        resB.username,
                    ...
                    this.restTeam
                        .findTeam(this.match.firstTeam.toString())
                        .subscribe(async (resA) => {
                            this.restTeam
                                .findTeam(this.match.secondTeam.toString())
                                .subscribe(async (resB) => {
                                    matches[mat.order - 1] = new myMatch(
                                        mat.id,
                                        mat.order,
    ...
    async generateSchedule() {
        ...
        const participants$ = this.restTournaments.getParticipants(
            this.myParam
        );
        ...
        for (let i = 0; i < this.participants.length; i += 2) {
            ...
            const $mat = this.restMatch.create(match).subscribe((res) => {
                ...
                if (
                    this.tournament.capacity / 2 ==
                    this.spider.eightfinals.length
                ) {
                    this.restTournaments
                        .updateSchedule(this.spider)
                        .subscribe(() => this.fillBracket());
                }
            });
        }
    }
    ...
    evaluate() {
        if (this.quarterMatches) {
            ...
            for (let i = 0; i <= this.eightMatches.length; i += 2) {
                if (
                    this.eightMatches[i]?.ScoreA !=
                        this.eightMatches[i]?.ScoreB &&
                    this.eightMatches[i + 1]?.ScoreA !=
                        this.eightMatches[i + 1]?.ScoreB &&
                    !this.quarterMatches[counter] &&
                    this.quarterMatches[counter]?.ScoreA ==
                        this.quarterMatches[counter]?.ScoreB
                ) {
                    this.quarterMatches[counter] = new myMatch(
                        0,
                        0,
                        "",
                        "",
                    ...
                    if (
                        ...
                        this.quarterMatches[counter].TeamA =
                            this.eightMatches[i].TeamA;
                        this.quarterMatches[counter].idA =
                            this.eightMatches[i].idA;
                    } else if (
                        this.eightMatches[i].ScoreA <
                        this.eightMatches[i].ScoreB
                    ) {
                        this.quarterMatches[counter].TeamA =
                            this.eightMatches[i].TeamB;
                        this.quarterMatches[counter].idA =
                            this.eightMatches[i].idB;
                    }
                    ...
                    if (
                        ...
                        this.quarterMatches[counter].TeamB =
                            this.eightMatches[i].TeamA;
                        this.quarterMatches[counter].idB =
                            this.eightMatches[i].idA;
                    } else if (
                        this.eightMatches[i + 1].ScoreA <
                        this.eightMatches[i + 1].ScoreB
        ...
        if (this.semiMatches) {
            ...
            for (let i = 0; i <= this.quarterMatches.length; i += 2) {
                if (
                    this.quarterMatches[i]?.ScoreA !=
                        this.quarterMatches[i]?.ScoreB &&
                    this.quarterMatches[i + 1]?.ScoreA !=
                        this.quarterMatches[i + 1]?.ScoreB &&
                    !this.semiMatches[counter] &&
                    this.semiMatches[counter]?.ScoreA ==
                        this.semiMatches[counter]?.ScoreB
                ) {
                    this.semiMatches[counter] = new myMatch(
                        0,
                        0,
                        "",
                        "",
                    ...
                    if (
                        ...
                        this.semiMatches[counter].TeamA =
                            this.quarterMatches[i].TeamA;
                        this.semiMatches[counter].idA =
                            this.quarterMatches[i].idA;
                    } else if (
                        this.quarterMatches[i].ScoreA <
                        this.quarterMatches[i].ScoreB
                    ) {
                        this.semiMatches[counter].TeamA =
                            this.quarterMatches[i].TeamB;
                        this.semiMatches[counter].idA =
                            this.quarterMatches[i].idB;
                    }
                    if (
                        this.quarterMatches[i + 1].ScoreA >
                        this.quarterMatches[i + 1].ScoreB
                    ) {
                        this.semiMatches[counter].TeamB =
                            this.quarterMatches[i].TeamA;
                        this.semiMatches[counter].idB =
                            this.quarterMatches[i].idA;
                    } else if (
                        this.quarterMatches[i + 1].ScoreA <
                        this.quarterMatches[i + 1].ScoreB
        ...
        if (
            this.final &&
            this.bronze &&
            this.semiMatches[0]?.TeamA &&
            this.semiMatches[1]?.TeamA &&
            this.final[0]?.TeamA == "" &&
            this.bronze[0]?.TeamA == "" &&
            ...
            if (
                this.semiMatches[0]?.ScoreA != this.semiMatches[0]?.ScoreB &&
        ...
        if (
            ...
            if (
                this.bronze &&
                this.quarterMatches[0]?.TeamA &&
                this.quarterMatches[1]?.TeamA &&
                this.bronze[0]?.TeamA == "" &&
                this.bronze[0]?.ScoreA == 0
            ) {
                this.bronze[0] = new myMatch(0, 0, "", "", 0, 0, 0, 0);
                if (
                    this.quarterMatches[0]?.ScoreA !=
                        this.quarterMatches[0]?.ScoreB &&
                    this.quarterMatches[1]?.ScoreA !=
                        this.quarterMatches[1]?.ScoreB
                ) {
                    if (
                        this.quarterMatches[0].ScoreA >
                        this.quarterMatches[0].ScoreB
