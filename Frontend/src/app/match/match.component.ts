...
export class MatchComponent implements OnInit {
    ...
    async onSubmit() {
        const $match = this.restMatch
            .getMatch(this.data.toString())
            .subscribe((res) => {
                if (res) {
                    if (
                        this.matchFormA.value.scoreA == null ||
                        this.matchFormB.value.scoreB == null ||
                        this.matchFormA.value.scoreA == "" ||
                        this.matchFormB.value.scoreB == ""
                    ) {
